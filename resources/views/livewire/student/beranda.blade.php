<div class="relative w-full pb-6 pt-2 min-h-[calc(100vh-4rem)]">
    ...
    <section class="flex items-center gap-4 mt-2 px-1">
        <div
            ...
            @if (!empty($student->avatar))
                <img alt="Student Profile Photo" class="w-full h-full object-cover"
                    src="{{ asset('storage/' . $student->avatar) }}" />
            @else
                <span class="text-[#3525cd] font-bold text-[24px]">
    ...
    <section class="flex flex-col gap-3 mt-6 px-1">
        ...
        <div class="bg-white rounded-2xl p-4 flex flex-col gap-3 shadow-sm border border-slate-100">
            <div class="flex items-center justify-between {{ !$isBiodataComplete ? 'group cursor-pointer' : '' }}"
                ...
                <span
                    class="material-symbols-outlined {{ $isBiodataComplete ? 'text-green-500' : 'text-slate-400 group-hover:text-slate-600' }}">{{ $isBiodataComplete ? 'check_circle' : 'chevron_right' }}</span>
            </div>
            ...
            <div class="flex items-center justify-between {{ !$isParentComplete ? 'group cursor-pointer' : '' }}"
                ...
                <span
                    class="material-symbols-outlined {{ $isParentComplete ? 'text-green-500' : 'text-slate-400 group-hover:text-slate-600' }}">{{ $isParentComplete ? 'check_circle' : 'chevron_right' }}</span>
            </div>
            ...
            <div class="flex items-center justify-between {{ !$isDudikaComplete ? 'group cursor-pointer' : '' }}"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-[13px] font-semibold text-slate-700">Data DUDIKA
                        {{ $isDudikaComplete ? 'lengkap' : 'belum lengkap (Isi Bid. Pekerjaan)' }}</span>
                </div>
                <span
                    class="material-symbols-outlined {{ $isDudikaComplete ? 'text-green-500' : 'text-slate-400 group-hover:text-slate-600' }}">{{ $isDudikaComplete ? 'check_circle' : 'chevron_right' }}</span>
            </div>
            ...
            <div class="flex items-center justify-between {{ !$isJurnalRevisiClean ? 'group cursor-pointer' : '' }}"
                ...
                <span
                    class="material-symbols-outlined {{ $isJurnalRevisiClean ? 'text-green-500' : 'text-slate-400 group-hover:text-slate-600' }}">{{ $isJurnalRevisiClean ? 'check_circle' : 'chevron_right' }}</span>
            </div>
        </div>
    </section>
    ...
    <section class="mt-6 px-1">
        <div
            class="relative overflow-hidden rounded-[1.25rem] bg-gradient-to-br from-[#4f46e5] to-[#3525cd] p-5 shadow-md flex flex-col gap-3">
            ...
            <div class="flex items-start justify-between relative z-10">
                <div class="flex flex-col gap-1">
                    <div
                        ...
                        <span class="text-[10px] font-extrabold text-white uppercase tracking-wider">Pengumuman</span>
                    </div>
                    <p class="text-[10px] font-medium text-indigo-100 opacity-80 mt-1 pl-1">Diunggah:
                        {{ now()->isoFormat('D MMM YYYY') }}</p>
                </div>
                <span
                    class="text-[9px] font-bold text-white bg-white/20 px-2 py-0.5 rounded border border-white/10">Terbaru</span>
            </div>

            <div class="relative z-10 mt-1 pl-1">
                <h3 class="text-[16px] font-extrabold text-white mb-1.5">Semangat Menjalani PKL!</h3>
                <p class="text-[12px] text-indigo-100 opacity-90 leading-relaxed font-medium">
                    Pastikan Anda selalu mengisi absen tepat waktu dan mencatat seluruh jurnal kegiatan harian Anda
                    dengan detail dan jujur. Jaga nama baik sekolah!
                </p>
            </div>
        </div>
    </section>
    ...
    <div class="fixed bottom-[88px] right-4 z-40">
        <a href="{{ route('siswa.absen') }}" wire:navigate
            class="flex h-14 w-14 items-center justify-center rounded-full bg-[#3525cd] text-white shadow-lg active:scale-90 hover:bg-[#2c1eb3] transition-all border-4 border-white">
